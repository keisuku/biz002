# モメンタム・イグニッション検出器

「凪から瞬間的に値動きの速度が急変した直後」を検出し、その方向へ乗って「速度が緩んだ瞬間」に
撤退するスキャル手法を、**まず統計的に成立するかどうか検証する**ためのリポジトリ。

* Phase 1 データ取得・整備（バッチ）— 実装済み
* Phase 2 発火イベント台帳と統計分析（バッチ）— 実装済み
* Phase 3 リアルタイム検出器と通知（VPS 常駐）— **未着手（着手しない）**

Phase 3 は「Phase 2 の生死判定（§4.1）を実データで通過した場合のみ」作る、という指示書の
条件を満たしていないため、コードは一行も書いていない。`src/live/` は存在しない。

---

## 現在の状態（重要）

2026-07-31 に、Binance 公開アーカイブへ接続できる環境で取得経路を復元し、
BTCUSDT / ETHUSDT の 2026-07-23〜29 を固定条件のまま流した。

**初期定義は生死判定を通過しなかった。Phase 3 は作らない。**

| Symbol | Events | 60s MFE median | Estimated round-trip cost | MFE / cost |
|---|---:|---:|---:|---:|
| BTCUSDT | 58 | 0.01127% (1.13 bps) | 0.13995% | 0.081x |
| ETHUSDT | 1 | 0.00054% (0.05 bps) | 0.13413% | 0.004x |

ETH は 1 件しかなく結論を出せるサンプル数ではない。BTC も 7 日間の工学的スパイクであり、
3 年検証の代わりではない。ただし「初期条件を見た後に調整せず、一度停止する」という
§4.1 / §9 の判断には十分である。詳細は
[`docs/REAL_DATA_REVIEW.md`](docs/REAL_DATA_REVIEW.md)。

### 別仮説: 特大の動きに乗ってストップ後は放置

2023-08-01〜2026-07-29の日足から、結果を見る前にレンジ上位8日と固定シードの
対照8日を選び、前後1日を含む46ユニーク日のaggTradesで `ride` を検証した。

* 全局面混合はBTC・ETHとも負
* 対照群は全24固定条件で負
* BTCの上位群だけは、10秒・6σ・24時間保有で平均 `+1.391R`
* ただし利益はショート・直前トレンド同方向に集中し、最大寄与窓を除くと `-0.079R`
* 現在の初動開始ラベルは未来参照で、最良の10秒窓を初動3秒後に約定することは実装不可能

したがって成立確定ではなく、「BTCの荒相場ショート」を未見データで再検証する価値が
残った段階。Phase 3には進まない。期間一覧、全層別、チェックサム結果を含む詳細は
[`docs/TREND_RIDE_REAL_DATA_REPORT.md`](docs/TREND_RIDE_REAL_DATA_REPORT.md)。

生データと生成 Parquet は Git に含めない。取得元、チェックサム、期間、軽量な集計結果だけを
再現可能な形で残す。

---

## セットアップ

```bash
pip install -r requirements.txt
python -m pytest tests -q          # 52 件
```

Python 3.11+ / Polars / NumPy / SciPy / PyArrow。データは Parquet。

## 実行手順

### Phase 1 — データ取得

```bash
# 1. バケットの実構造を取得して reports/data_layout.json に保存する
#    （指示書のパスは起点にすぎない。実物と食い違えば実物に合わせる）
python -m src.cli discover --symbol BTCUSDT

# 2. 取得（aggTrades / klines / metrics / bookTicker）
python -m src.cli download --symbol BTCUSDT --start 2023-08-01 --end 2026-07-29

# 3. 1 秒バーへの集約 + 欠損期間レポート
python -m src.cli bars --symbol BTCUSDT --start 2023-08-01 --end 2026-07-29

# 4. is_buyer_maker の極性検証（急騰局面でテイカー買いが優勢になるか）
python -m src.cli verify-flags --symbol BTCUSDT --start 2023-08-01 --end 2026-07-29
```

### Phase 2 — 台帳と分析

```bash
python -m src.cli refs   --symbol BTCUSDT --start ... --end ...   # 凪判定用の参照系列
python -m src.cli events --symbol BTCUSDT --start ... --end ...   # 発火イベント台帳
python -m src.cli funnel --symbol BTCUSDT --start ... --end ...   # 4条件のどれが律速か
python -m src.cli impulse-scan --symbol BTCUSDT --start ... --end ...  # 初動エントリの天井
python -m src.cli ride --symbol BTCUSDT --start ... --end ...      # 乗って放置する手法（R倍率）
python -m src.cli analyze --symbol BTCUSDT --tick-stops           # §4.1〜§4.5
python -m src.cli latency --symbol BTCUSDT                        # 0〜20秒の反応遅延
```

`analyze` は **§4.1 の生死判定で FAIL なら、そこで止まる**。
時間帯分析にも出口シミュレーションにも進まない（`--force` を明示すれば続行できるが、
指示書 §9 のとおり、まず数字を見て判断すること）。

生死判定を通過した場合のみ:

```bash
python -m src.cli validate --symbol BTCUSDT --start ... --end ...  # §6 の 1〜6
python -m src.cli trials --n 3 --note "手動でしきい値を調整した回数"  # §9 手動調整も加算する
```

### 合成データ（コード検証専用）

```bash
python -m src.cli synth --symbol SYNTHSTRONG --start 2025-01-01 --end 2025-03-01 --profile strong
```

`--profile` は `realistic` / `strong` / `null` の 3 つ。いずれも人工データであり、
**ここから出る数字は手法の証拠にはならない**（`src/data/synthetic.py` 冒頭に明記）。

---

## 設計上の判断

### 板データの非対称性（§2.3）
コア検出ロジックは **aggTrades と klines だけ**で構成する。バックテストと本番で
同じ特徴量が使えるようにするため。板の消失は次の代理変数で表現する:

* 単位出来高あたりの価格インパクト `impact = |10秒価格変化率| / 10秒出来高`
* 連続する約定間の価格ギャップ `gap_max_bps`（1 秒バー生成時に算出）
* `bookTicker` 由来のスプレッド（あれば `spread_bps` 列として使用、無ければギャップで代替）

実板（`depth`）は Phase 3 のオプション確認用であって、必須条件にはしない。

### ルックアヘッドの防止（§6.7）
特徴量に使ってよい時系列操作は後ろ向き窓 `rolling_*` と過去方向 `shift(+n)` のみ。
参照統計（`sigma_ref` / `tick_ref` / `impact_ref` / 凪の分位点）は発火そのものを含まないよう
必ず窓の分だけ過去にずらす。`tests/test_lookahead.py` が全特徴量に対して
「未来を足すと値が変わるか」を機械的に検査する。

### 時刻・単位の扱い
* Binance は配布 CSV のヘッダ有無と時刻粒度（ms / µs）を途中で変えている。パーサは
  ファイルごとに判定する。決め打ちすると 1000 倍ずれた「未来」のデータになる。
* `is_buyer_maker == true` は買い手がメイカー、すなわち**テイカーは売り**。
  反転させると全ての結論が逆になるため、実データでの検証コマンドを用意している。
* 1 秒バーは常に密（1 日 86400 行）。ローリング窓の「行数 = 秒数」を保証するため。
  約定が無い秒は `is_filled=True` で記録し、長い欠損の周辺では発火させない。

### 単位の約束
`mfe_H` / `mae_H` / `ret_H` / `*_pct` は**パーセント (%)**、`*_bps` は bps (0.01%)。
MFE >= 0、MAE <= 0（一度も順行/逆行しなければ 0）。

---

## 検証の状態

`pytest tests` = 101 件。内訳:

| 対象 | 内容 |
|---|---|
| ルックアヘッド | 全特徴量に対する「未来を足しても値が変わらない」検査、参照窓が発火を含まないこと、MFE が発火した秒の高値を含まないこと |
| 特徴量の値 | rv / velocity / tick_ratio / ofi_ratio / impact の定義どおりの算出、欠損区間の除外、凪判定 |
| Phase 1 集約 | OHLCV・テイカー売買の分離・空秒の穴埋め・ギャップ算出・`is_buyer_maker` 極性検査が反転を検出すること |
| パーサ | ヘッダ有無 3 形式、秒/ms/µs/ns の正規化、数値フラグ、列数超過、zip、チェックサム長判定 |
| 発火・結果・出口 | クールダウンの統合、4 条件の AND、MFE/MAE の符号、往復コスト、E1〜E5 の発動条件、ハードストップ優先、最大保有時間 |
| §6 の手続き | fold の連続性、訓練期間の選択が凍結されること、DSR が試行回数で罰されること、ブロックブートストラップ、プラトー判定が尖ったピークを棄却すること、試行回数ログ |
| 結合テスト | 合成データに埋め込んだ発火だけを正しい向きで検出すること（適合率 100%）、チャンク幅が結果を変えないこと、継続順行の無い世界で生死判定が FAIL を返すこと |
| 手動執行遅延 | 0〜20秒後の実行可能価格からMFE・収益を再計算し、速度窓を含む初動年齢を明示すること |
| 集約のルックアヘッド | その日の最初の約定より前の秒に、未来の価格が入らないこと。前日終値の引き継ぎ |
| ファネル診断 | どの条件が律速かの判定、チャンク加算、閾値がどの分位点に当たるかの算出 |
| 乗って放置 | ストップ発動で正確に -1R、伸びれば +NR、逆行が先ならその後いくら伸びても -1R、中央値が負でも平均 R が正なら成立と判定すること、順張り/逆張りの層別 |
| 初動天井測定 | 開始秒のラベルが「その先の値動き」に依存しないこと、エントリ価格が age 秒後の実勢値であること、MFE基準を通っても純リターンが負なら明示すること |

### イベント件数が銘柄間で大きく違うとき

閾値を触る前に `funnel` を見る。4 条件を **個別に** 通過した秒数と、**その条件だけを外した**
ときの通過数（`binding_factor`）が出るので、どこで落ちているかが一目で分かる。
併せて、凪の秒における各比率の分布と、**設定した閾値がその分布の何パーセンタイルに
当たるか**を出す。同じ `impact >= 3.0` でも銘柄によって厳しさが全く違うため、
「同じ閾値なのに件数が桁違い」が定義差なのか分布差なのかをここで切り分ける。

この診断は閾値を変更しないので、§9 の試行回数には数えない。

### 「もっと速く入れば勝てるのか」を、検出器を作らずに調べる

`impulse-scan` は **初動の開始秒を後知恵でラベルし**、そこから `age` 秒後の
実勢終値で入った場合の成果を測る。`age = 検出窓 + 通知遅延 + 人間の反応`。

* `age=0` は物理的に不可能な理想値。**どんな検出器もこれを超えられない天井**
* 天井がコストに届かなければ、検出器をいくら速くしても勝てない。そこで終わり
* 届いても勝てる証拠ではない（MFE は最良の瞬間に降りられた場合の幅）。
  実時間検出器を検討してよい、というだけ

**これはバックテストではない。** 開始秒の特定に未来を使っているので実行不可能である。
価格はすべて実データの実勢値で、未来を使っているのは「どの秒を選ぶか」の一点だけ。
初動の大きさは開始から `window` 秒だけで測り、その先の値動きは使わない
（結果的に伸びた動きだけを拾うと、勝ち馬を選んだ天井になる）。

### 「乗って放置」は別の手法として測る（`ride`）

指示書はスキャル前提（60 秒・MFE 中央値・凪条件）で書かれているが、
**「特大の動きに乗って、ストップを置いて放置する」手法はそれとは別物**であり、
同じ指標で判定すると構造的に必ず不合格になる。理由と対処:

| | 指示書のスキャル | `ride` |
|---|---|---|
| 保有 | 60 秒 / 勢いが緩んだら降りる | 損切りに掛かるまで放置（15分〜24時間） |
| 指標 | MFE の**中央値** | **R 倍率の平均・合計・右の裾** |
| 前提 | 凪（過去 7 日で最低ボラ） | **凪条件を使わない**（荒れた局面で効く手法） |
| コスト | 支配的（値幅 1bps に対し 14bps） | ほぼ無視できる（値幅 1〜3% に対し 14bps） |

損益が「多くは −1R、たまに +10R」の形になるため、**中央値はその『たまに』を
定義上捨てる**。中央値で判定するとトレンドフォローは必ず落ちる。

裁量部分（「そろそろ調整が来るから逆方向」）は検出器に直接コピーできないので、
計算可能な形に置き換えて層別する:

* `trend_align` … 初動の向きが直前 1 時間のトレンドと同じか逆か
* `trend_z` … 直前トレンドがどれだけ伸び切っているか
* `vol_z` … 実現ボラが過去 7 日の水準からどれだけ離れているか（暴落期かどうか）

### `ride` の 2 つの必須検査

`ride` は R 倍率の集計に加えて、次の 2 つを必ず出す。**どちらかを落としたら結論は無効。**

**検査1: シグナル vs ランダム時刻**

同じ期間・同じ方向分布・同じストップ幅で、**時刻だけランダム**に入る対照群を作る。
荒れた日（多くは暴落日）を選んでショートを取れば、初動の検出に価値が無くても
プラスになる。単に下げ相場でショートを持っていただけだからだ。
**シグナルがランダムに勝っていなければ、勝っていたのは局面であってシグナルではない。**

**検査2: 最大寄与ブロックの leave-one-out**

3 日窓を 1 つ取り除いたときに平均 R がどうなるかを出す。
1 窓抜いてプラスが消えるなら実効サンプル数はほぼ 1 であり、偶然と区別できない。

**局面依存の手法なので、全期間の平均だけを見てはいけない。** 凪の期間のノイズに
薄められて消える。層別表（`trend_ride_strata.csv`）が結論。

---

## ディレクトリ

```
config/          params.yaml（全閾値）, symbols.yaml
src/
  data/          download.py（構造を実行時に取得）, to_seconds.py, verify_flags.py, synthetic.py
  features.py    §3.2 特徴量
  events.py      §3.3 発火抽出
  outcomes.py    §3.4 MFE/MAE とコスト
  exits.py       §4.4 出口ルール E1〜E5
  analysis/      viability.py(§4.1-4.2) heatmap.py(§4.3) stop_fill.py(§4.5) validation.py(§6)
  pipeline.py    チャンク処理と格子探索
  cli.py         コマンド入口
tests/           52 件
reports/         生成物（CSV / PNG / JSON）
docs/            PHASE2_REPORT.md
```

閾値はすべて `config/params.yaml` にある。コード内にマジックナンバーは置かない。

## やらないこと

1. ダッシュボード・チャート UI を作らない（出力は通知とログのみ。分析結果は静的な CSV/PNG）
2. 自動発注は実装しない
3. 価格ベースのトレーリングストップを使わない
4. RSI 等のオシレーターを使わない
5. 有利な時間帯を定数としてハードコードしない
6. 生死判定を通過していない手法に Phase 3 を作らない
